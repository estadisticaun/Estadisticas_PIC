...
author: "<br>Rectoría<br>Vicerrectoría Académica<br>Universidad Nacional de Colombia<br>Julio de 2025"
format:
  onyxia-revealjs:
...
# Admitidos Pregrado 20262

# General
Admitidos_262 <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2) |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Clase = "Admitidos")

# Bogotá
Admitidos_262_Bog <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2, SEDE_NOMBRE == "Bogotá") |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Clase = "Admitidos")

# Medellín
Admitidos_262_Med <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2, SEDE_NOMBRE == "Medellín") |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Clase = "Admitidos")

# Manizales
Admitidos_262_Man <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2, SEDE_NOMBRE == "Manizales") |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Clase = "Admitidos")

# Palmira

Admitidos_262_Pal <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2, SEDE_NOMBRE == "Palmira") |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Clase = "Admitidos")
...
Admitidos_262_Paz <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2, SEDE_NOMBRE == "De La Paz") |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Clase = "Admitidos")
...
Historico_Programas <- read_excel("Datos/Historico_Programas_1.xlsx", guess_max = 1000) %>% filter(TIPO_NIVEL_PRO == "Pregrado")

# Bases de poblaciones

# Serie General: Admitidos - Mat - Cupos - Graduados

Admitidos <- AdmitidosPre %>% filter(YEAR >= 2021) %>% summarise(Total = n(), .by = c(YEAR, SEMESTRE)) %>% mutate(Clase = "Admitidos") %>% relocate(Clase, .before = Total) |> bind_rows(Admitidos_262)
...
Admitidos_Pro_Bog <- Admitidos_Pro %>% filter(SEDE_PROG == "Bogotá") %>% summarise(Total = n(), .by = c(YEAR, SEMESTRE)) %>% mutate(Clase = "Admitidos") %>% relocate(Clase, .before = Total) |> bind_rows(Admitidos_262_Bog)
...
Admitidos_Pro_Med <- Admitidos_Pro %>% filter(SEDE_PROG == "Medellín") %>% summarise(Total = n(), .by = c(YEAR, SEMESTRE)) %>% mutate(Clase = "Admitidos") %>% relocate(Clase, .before = Total) |>  bind_rows(Admitidos_262_Med)
...
Admitidos_Pro_Man <- Admitidos_Pro %>% filter(SEDE_PROG == "Manizales") %>% summarise(Total = n(), .by = c(YEAR, SEMESTRE)) %>% mutate(Clase = "Admitidos") %>% relocate(Clase, .before = Total) |> bind_rows(Admitidos_262_Man)
...
Admitidos_Pro_Pal <- Admitidos_Pro %>% filter(SEDE_PROG == "Palmira") %>% summarise(Total = n(), .by = c(YEAR, SEMESTRE)) %>% mutate(Clase = "Admitidos") %>% relocate(Clase, .before = Total)|> bind_rows(Admitidos_262_Pal)
...
Admitidos_Pro_Paz <- Admitidos_Pro %>% filter(SEDE_PROG == "De La Paz") %>% summarise(Total = n(), .by = c(YEAR, SEMESTRE)) %>% mutate(Clase = "Admitidos") %>% relocate(Clase, .before = Total)|>  bind_rows(Admitidos_262_Paz)
...
Agregar(TOTAL ~ YEAR + SEMESTRE,
                    ...
                    datos = AdmitidosPre,
                    textNA = "Sin información") %>% filter(Clase == "Total") %>%
              bind_rows(Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2) |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE)) |> 
  mutate(Variable = "TOTAL", Clase = "Total")) |> 
              Plot.Series(categoria = "TOTAL",
               colores = c("#8cc63f"), # verde, Total
...
```
<p style="font-size: 12px;">Fuente: Estadísticas oficiales DNPE</p>
<p style="font-size: 12px;"><b>Nota:</b> La información de estudiantes admitidos en el periodo 2026-2 aún no se encuentra publicada y disponible en las estadísticas oficiales</p>
...
Agregar(INS_SEDE_NOMBRE ~ YEAR + SEMESTRE,
                  frecuencia = list(c(2021:2026), c(1:2)),
                    ...
                    textNA = "Sin información") %>%
  bind_rows(Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2) |> 
  summarise(Total = sum(ADMITIDOS), .by = c(YEAR, SEMESTRE, SEDE_NOMBRE)) |> 
  rename(Clase = SEDE_NOMBRE) |> 
  mutate(Variable = "INS_SEDE_NOMBRE")) |> 
           Plot.Series(categoria = "INS_SEDE_NOMBRE",
...
```{r}
Admitidos_Pre_262 <- Cupos_Pre |> filter(YEAR == 2026,	SEMESTRE == 2) 
```
...
datos <- Admitidos_Pre_262 %>% 
  filter(SEDE_NOMBRE == "Bogotá") %>% 
  summarise(Admitidos = sum(ADMITIDOS), Cupos = sum(CUPOS),
            .by = c(COD_PADRE, NOMBRE_PROGRAMA)) %>% 
  slice_max(Admitidos, n = 15) 

  highchart() %>%
   hc_xAxis(categories = datos$NOMBRE_PROGRAMA, 
           title = list(text = "Programa Académico")) %>%
  hc_yAxis(title = list(text = "Total admitidos")) %>%
  
  # Añadir la serie de barras horizontales (Total)
  hc_add_series(
    data = datos$Admitidos,
    type = "bar",
    name = "Admitidos",
    color = "#a8ddb5",
     dataLabels = list(
      enabled = TRUE,
      align = "left",        # Alinea el texto al extremo derecho (final) de la barra
      inside = FALSE,         # Mantiene el texto justo por dentro de la barra
      size = 30,
      #color = "#FFFFFF",     # Color de fuente blanco para contrastar con la barra azul
  ...
  hc_add_series(
    ...
    name = "Cupos",
    marker = list(
      symbol = "diamond", # Define la forma (puedes cambiarlo a "circle", "square", etc.)
      radius = 7,       # Tamaño del punto/estrella
      fillColor = "#de2d26" # Color del punto/estrella
    )
  ) %>%
    ...
    buttons = list(
      contextButton = list(
        align = "right",      # Alineación horizontal a la derecha
        verticalAlign = "top" # Alineación vertical arriba
      )
    )
  ) %>%
  # Opciones estéticas adicionales
  hc_title(text = "Top 15 programas de pregrado con mayor cantidad de admitidos vs cupos disponibles en la Sede Bogotá, Periodo 2026-2") %>%
  ...
  #   vertical = FALSE,
  #   colores   = rep("#a8ddb5", 16),
  #   titulo = "Top 15 programas de pregrado con mayor cantidad de admitidos en la Sede Bogotá, Periodo 2026-2",
...
```
<p style="font-size: 12px;">Fuente: Estadísticas oficiales DNPE</p>
<p style="font-size: 12px;"><b>Nota 1:</b> La información de estudiantes admitidos en el periodo 2026-2 aún no se encuentra publicada y disponible en las estadísticas oficiales</p>
<p style="font-size: 12px;"><b>Nota 2:</b> El total de cupos por programa corresponde a los definidos, para el periodo 2026-2, por los Consejos de Facultad. No incluye los cupos PEAMA</p>
...
highchart() %>%
  ...
  hc_add_series(
    ...
    type = "bar",
    name = "Admitidos",
    color = "#a8ddb5", # Puedes personalizar el color de la barra aquí
    dataLabels = list(
      enabled = TRUE,
      align = "left",        # Alinea el texto al extremo derecho (final) de la barra
      inside = FALSE,         # Mantiene el texto justo por dentro de la barra
  ...
  # Añadir la serie de puntos/estrellas sobrepuestos (Cupos)
  hc_add_series(
    data = datos$Cupos,
    type = "scatter",
    name = "Cupos",
  ...
  # Opciones estéticas adicionales
  hc_title(text = "Top 15 programas de pregrado con menor cantidad de admitidos vs cupos disponibles en la Sede Bogotá, Periodo 2026-2") %>%
  hc_legend(enabled = TRUE) %>%
  hc_tooltip(shared = TRUE)
  ...
  #   titulo = "Top 15 programas de pregrado con menor cantidad de admitidos en la Sede Bogotá, Periodo 2026-2",
  #   labelY = "Total admitidos",
  #   labelX = "Programa Académico")
...
highchart() %>%
  ...
  # Opciones estéticas adicionales
  hc_title(text = "Total de admitidos vs cupos disponibles por programas académicos de pregrado en la Sede Medellín, Periodo 2026-2") %>%
  hc_legend(enabled = TRUE) %>%
  hc_tooltip(shared = TRUE)
...
# Admitidos_Pre_262 %>% 
#   filter(SEDE_NOMBRE == "Medellín") %>% 
#   summarise(Total = sum(ADMITIDOS), .by = c(COD_PADRE, NOMBRE_PROGRAMA)) %>% 
#   Plot.Barras(
#     valores   = Total,
#     categoria = NOMBRE_PROGRAMA ,
#     vertical = FALSE,
#     colores   = rep("#7fcdbb", 29),
#     titulo = "Total de admitidos a la Sede Medellín por programas académicos, Periodo 2026-2",
#     labelY = "Total admitidos",
#     labelX = "Programa Académico")
...
highchart() %>%
  ...
  # Opciones estéticas adicionales
  hc_title(text = "Total de admitidos vs cupos disponibles por programas académicos de pregrado en la Sede Manizales, Periodo 2026-2") %>%
  hc_legend(enabled = TRUE) %>%
  hc_tooltip(shared = TRUE)
...
#     categoria = NOMBRE_PROGRAMA ,
#     vertical = FALSE,
#     colores   = rep("#7fcdbb", 14),
#     titulo = "Total de admitidos a la Sede Manizales por programas académicos, Periodo 2026-2",
#     labelY = "Total admitidos",
#     labelX = "Programa Académico")
...
highchart() %>%
  ...
  # Opciones estéticas adicionales
  hc_title(text = "Total de admitidos vs cupos disponibles por programas académicos de pregrado en la Sede Palmira, Periodo 2026-2") %>%
  hc_legend(enabled = TRUE) %>%
  hc_tooltip(shared = TRUE)
...
#     categoria = NOMBRE_PROGRAMA ,
#     vertical = FALSE,
#     colores   = rep("#7fcdbb", 9),
#     titulo = "Total de admitidos a la Sede Palmira por programas académicos, Periodo 2026-2",
#     labelY = "Total admitidos",
#     labelX = "Programa Académico")
```
<p style="font-size: 12px;">Fuente: Estadísticas oficiales DNPE</p>
...
highchart() %>%
  ...
  # Opciones estéticas adicionales
  hc_title(text = "Total de admitidos vs cupos disponibles por programas académicos de pregrado en la Sede La Paz, Periodo 2026-2") %>%
  hc_legend(enabled = TRUE) %>%
  hc_tooltip(shared = TRUE)
...
#     titulo = "Total de admitidos a la Sede La Paz por programas académicos, Periodo 2026-2",
#     labelY = "Total admitidos",
#     labelX = "Programa Académico")
...
```
<p style="font-size: 13px;"><b>Cupos:</b> Los cupos en pregrado de la Universidad se calculan a partir de lo definido en los Consejos de Facultad. No incluye los cupos PEAMA</p>
...
```
<p style="font-size: 13px;">Fuente: Dirección Nacional de Planeación y Estadística - DNPE</p>
<p style="font-size: 13px;"><b>Cupos:</b> El total de cupos en pregrado de la Universidad se calcula a partir de lo definido en los Consejos de Facultad. No incluye los cupos PEAMA</p>
